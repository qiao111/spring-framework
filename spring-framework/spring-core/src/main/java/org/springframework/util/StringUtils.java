...
public abstract class StringUtils {
	...
	public static String cleanPath(String path) {
		if (!hasLength(path)) {//path = D:/user/study/spring-beans.xml
			return path;
		}
		String pathToUse = replace(path, WINDOWS_FOLDER_SEPARATOR, FOLDER_SEPARATOR);// D:/user/study/spring-beans.xml

		// Strip prefix from path to analyze, to not treat it as part of the
		// first path element. This is necessary to correctly parse paths like
		// "file:core/../core/io/Resource.class", where the ".." should just
		// strip the first "core" directory while keeping the "file:" prefix.
		int prefixIndex = pathToUse.indexOf(":"); // 1
		String prefix = "";
		if (prefixIndex != -1) {
			prefix = pathToUse.substring(0, prefixIndex + 1);//D:
			if (prefix.contains("/")) {
				prefix = "";
			}
			else {
				pathToUse = pathToUse.substring(prefixIndex + 1);//  /user/study/spring-beans.xml
			}
		}
		if (pathToUse.startsWith(FOLDER_SEPARATOR)) {
			prefix = prefix + FOLDER_SEPARATOR;//  D:/user/study/spring-beans.xml
			pathToUse = pathToUse.substring(1);//  :/user/study/spring-beans.xml
		}

		String[] pathArray = delimitedListToStringArray(pathToUse, FOLDER_SEPARATOR);// [:,user,study,spring-beans.xml]
