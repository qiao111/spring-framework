...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	 ...
	 * 使用指定的beanFactory创建一个XmlBeanDefinitionReader
	 *   初始化资源加载器和环境变量
	 */
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		if (currentResources == null) {
			...
			this.resourcesCurrentlyBeingLoaded.set(currentResources);//默认4个
		}
		if (!currentResources.add(encodedResource)) {//添加资源文件
			throw new BeanDefinitionStoreException(
					"Detected cyclic loading of " + encodedResource + " - check your import definitions!");
		}
		try {
			InputStream inputStream = encodedResource.getResource().getInputStream();//转换为输入流
			try {
				InputSource inputSource = new InputSource(inputStream);
				if (encodedResource.getEncoding() != null) {
					inputSource.setEncoding(encodedResource.getEncoding());
				}
				return doLoadBeanDefinitions(inputSource, encodedResource.getResource());//真正加载解析xml文件
			}
			finally {
				inputStream.close();
			}
			...
			currentResources.remove(encodedResource);//移除当前资源文件
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		//创建bean定义文档阅读器  DefaultBeanDefinitionDocumentReader
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();
		int countBefore = getRegistry().getBeanDefinitionCount();//注册前已经注册好的
		documentReader.registerBeanDefinitions(doc, createReaderContext(resource));
		return getRegistry().getBeanDefinitionCount() - countBefore;//本次注册的
	}
