...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		String id = ele.getAttribute(ID_ATTRIBUTE);//id属性 bean
		String nameAttr = ele.getAttribute(NAME_ATTRIBUTE);//name属性 bean1,bean2

		List<String> aliases = new ArrayList<>();
		if (StringUtils.hasLength(nameAttr)) {
			String[] nameArr = StringUtils.tokenizeToStringArray(nameAttr, MULTI_VALUE_ATTRIBUTE_DELIMITERS);
			aliases.addAll(Arrays.asList(nameArr));// name = bean1,bean2
		}

		String beanName = id;
		if (!StringUtils.hasText(beanName) && !aliases.isEmpty()) {
			beanName = aliases.remove(0);//bean1
		...
		if (containingBean == null) {
			checkNameUniqueness(beanName, aliases, ele);//将name属性 bean1 bean2 加入到已使用的map中
		}
		//解析标签并将其分装在bd中
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		try {
			AbstractBeanDefinition bd = createBeanDefinition(className, parent);//定义bean  GenericBeanDefinition
			//解析bean的属性 并设置到GenericBeanDefinition中
	...
	public Object parsePropertyValue(Element ele, BeanDefinition bd, @Nullable String propertyName) {
		...
		boolean hasRefAttribute = ele.hasAttribute(REF_ATTRIBUTE);//引用
	...
	public Object parsePropertySubElement(Element ele, @Nullable BeanDefinition bd, @Nullable String defaultValueType) {
		if (!isDefaultNamespace(ele)) {
			return parseNestedCustomElement(ele, bd);//自定义标签
		}
